...
NaiveBayesHyperplane::NaiveBayesHyperplane(const HyperParameters &hp) : NaiveBayes( hp )
{
    m_featureType = (hp.naiveBayesFeatureType == "Gaussian") ? FEATURE_GAUSSIAN : FEATURE_HISTOGRAM;
}

void NaiveBayesHyperplane::calcMeanAndVariance(const matrix<float>& data, const std::vector<int>& labels, FeatureHyperplane& f)
{
...
void NaiveBayesHyperplane::calcMeanAndVariance(const matrix<float>& data, const std::vector<int>& labels,
                                               const std::vector<double>& weights, FeatureHyperplane& f)
{
    std::vector<double> mean(m_hp.numClasses,0.0);
    std::vector<double> variance(m_hp.numClasses,0.0);
...
void NaiveBayesHyperplane::calcHistogram(const matrix<float>& data, const std::vector<int>& labels, FeatureHyperplane& f)
{
...
void NaiveBayesHyperplane::calcHistogram(const matrix<float>& data, const std::vector<int>& labels,
                                         const std::vector<double>& weights, FeatureHyperplane& f)
{
    std::vector<float> projectionVals(data.size1(),0.0);
    int featureCounter = 0;
...
void NaiveBayesHyperplane::train(const matrix<float>& data, const std::vector<int>& labels,
    ...
    for (int n = 0; n < m_hp.numRandomFeatures;n++)
    {
        ...
        if (m_featureType == FEATURE_GAUSSIAN)
        {
            ...
            for (int t = 0; t < m_hp.numTries;t++)
            {
                ...
                // calc error
                double error = 0;
                for (int sample = 0; sample < m_hp.numLabeled;sample++)
                {
                    //evaluate
...
// Weighted Training
void NaiveBayesHyperplane::train(const matrix<float>& data, const std::vector<int>& labels, const std::vector<double>& weights,
                                 matrix<float>& forestConfidences, matrix<float>& forestOutOfBagConfidences,
...
// Weighted Training
void NaiveBayesHyperplane::retrain(const matrix<float>& data, const std::vector<int>& labels, const std::vector<double>& weights,
                                 matrix<float>& forestConfidences, matrix<float>& forestOutOfBagConfidences,
                                 std::vector<int>& forestOutOfBagVoteNum, bool init)
{
    // Initialize
...
void NaiveBayesHyperplane::evalOutOfBagSamples(const matrix<float>& data)
{
}

void NaiveBayesHyperplane::eval(const matrix<float>& data, const std::vector<int> labels)
{
    ...
    std::vector<FeatureHyperplane>::iterator it(m_features.begin());
    std::vector<FeatureHyperplane>::iterator end(m_features.end());
    for (int sample = 0; sample < (int)labels.size(); sample++)
    {
        it = m_features.begin();
        while (it != end)
...
void NaiveBayesHyperplane::eval(const matrix<float>& data, matrix<float>& confidences)
{
    m_confidences.resize(data.size1(), m_hp.numClasses);
    // init to one (due to multiplication)
    for (int sample = 0; sample < (int)data.size1(); sample++)
...
void FeatureHyperplane::eval(const matrix<float>& data, const int sampleIndex, matrix<float>& confidences)
{
...
void FeatureHyperplane::evalGaussian(const matrix<float>& data, const int sampleIndex, matrix<float>& confidences)
{
    double projectionVals = 0.0;
    int featureCounter = 0;
...
void FeatureHyperplane::evalHistogram(const matrix<float>& data, const int sampleIndex, matrix<float>& confidences)
{
    double projectionVals = 0.0;
    int featureCounter = 0;
    BOOST_FOREACH(int feat, indeces)
