...
int main(int argc, char * argv [])
{
    ...
    DeformationFieldPointerType deformation=TransfUtils<ImageType>::createEmpty(FilterUtils<ImageType>::LinearResample(targetImg,0.02,false));
    ...
    for (int iter=0;iter<10;++iter){
        ...
        TransfUtils<ImageType,float,double>::OutputDeformationFieldPointerType dblDef=TransfUtils<ImageType,float,double>::cast(deformation);
        ...
        metric->SetFixedImage(fimg2);
        metric->SetMovingImage(fimg1);
        ...
        int numberOfPixels=deformation->GetBufferedRegion().GetNumberOfPixels();
        ...
        for (defIt.GoToBegin();!defIt.IsAtEnd();++defIt,++p){
            ...
            defIt.Set(defIt.Get()+disp);
            
        }
        //ImageUtils<DeformationFieldType>::writeImage("derivative.mha",deriv);
        LOG<<targetImg->GetLargestPossibleRegion().GetSize()<<" "<<movingedImg->GetLargestPossibleRegion().GetSize()<<endl;
        ImageUtils<ImageType>::writeImage(outputFilename,TransfUtils<ImageType>::warpImageWithReference(ImageType::ConstPointer(movingedImg),deformation,targetImg));
    }
}
