...
*/
tuple<UCharImagePtr, FloatImagePtr, FloatImagePtr>
compute(
    ShortImagePtr inputCT,
    float sigmaSmallScale,
    vector<float> sigmasLargeScale, char* softTissFilename= NULL
) {

    UCharImagePtr roi;
    FloatImagePtr softTissueEstimation;
...

    {
        ...
        FloatImagePtr tmpPtr=FilterUtils<ShortImage,FloatImage>::cast(thresholdedInputCT);
        FloatImagePtr smallScaleSheetnessImage =
            multiscaleSheetness(tmpPtr, scales);

        if(!softTissFilename){
        log("Estimating soft-tissue voxels");
        softTissueEstimation =  FilterUtils<UIntImage,FloatImage>::binaryThresholding(
                FilterUtils<UIntImage>::relabelComponents(
                    FilterUtils2<UCharImage, UIntImage>::connectedComponents(
            ...
            log("Loading soft-tissue voxels");
            //assume there is BONE probabilities in the file!!
            softTissueEstimation=FilterUtils<FloatImage>::linearTransform(ImageUtils<FloatImage>::readImage(softTissFilename),-1,-1);
            //softTissueEstimation=FilterUtils<FloatImage>::binaryThresholding(ImageUtils<FloatImage>::readImage(softTissFilename),0,0.5);
        }
        ...
        roi = FilterUtils<FloatImage,UCharImage>::binaryThresholding(
                                                                     chamferDistance(boneEstimation),0, 1000);
                                                                     //chamferDistance(softTissueEstimation),0, 30);
    }
    ...
    ImageUtils<FloatImage>::writeImage("test.nii",inputCTUnsharpMasked);
    log("Computing multiscale sheetness measure and blabla at %d scales")
