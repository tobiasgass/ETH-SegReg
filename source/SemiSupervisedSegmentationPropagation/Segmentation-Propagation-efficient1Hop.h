/**
 * @file   Segmentation-Propagation-efficient1Hop.h
 * @author Tobias Gass <tobiasgass@gmail.com>
 * @date   Thu Mar 12 14:31:22 2015
 * 
 * @brief  SSSP as described in the paper. 1hop is efficient
 * 
 * 
 */
#pragma once
...
public:
    int run(int argc, char ** argv){
        ...
        if (imageFileListAtlas != ""){
            ...
            atlasIDMap=atlasSegmentationIDMap;
        }
        ...
        for (ImageListIteratorType targetImageIterator=targetImages->begin();targetImageIterator!=targetImages->end();++targetImageIterator){                //iterate over targets
            ...
            if (atlasSegmentationIDMap->find(targetID)==atlasSegmentationIDMap->end()){ //do not calculate segmentation for atlas images
                ...
                for (ImageListIteratorType atlasIterator=inputAtlasSegmentations->begin();
                    ...
                    {//iterate over atlases
                        ...
                        if (atlasID!=targetID){
                            ...
                            DeformationFieldPointerType atlasTargetDeformation;
                            if (dontCacheDeformations){
                                atlasTargetDeformation = ImageUtils<DeformationFieldType>::readImage(deformationFilenames[atlasID][targetID]);
                            }else{
                                atlasTargetDeformation = deformationCache[atlasID][targetID];
                            }
                            ...
                            if (weighting==UNIFORM || metric == NONE ){
                                ...
                                if (weighting==GLOBAL){
                                    updateProbabilisticSegmentationGlobalMetric(probabilisticTargetSegmentation,probAtlasSegmentation,weight,targetImage,atlasImage,atlasTargetDeformation,metric);
                                }else if (weighting==LOCAL){
                                    updateProbabilisticSegmentationLocalMetricNew(probabilisticTargetSegmentation,probAtlasSegmentation,weight,targetImage,atlasImage,atlasTargetDeformation,metric);
                                }
                            }
                            
                            int intermediateN=0;
                            //propagate atlas segmentations via intermediate targets
                            for (ImageListIteratorType intermediateImageIterator=targetImages->begin();intermediateImageIterator!=targetImages->end() && (intermediateN<useNTargets);++intermediateImageIterator){//iterate over intermediate images
                                ...
                                if ( (intermediateID!=targetID) &&
                                     (intermediateID!=atlasID) && 
                                     (atlasSegmentationIDMap->find(intermediateID) == atlasSegmentationIDMap->end()) ){
                                    LOGV(3)<<VAR(targetID)<<" "<<VAR(atlasID)<<" "<<VAR(intermediateID)<<endl;

                                    ++intermediateN;
                                    DeformationFieldPointerType deformation;
                                    if (dontCacheDeformations){
                                        deformation = ImageUtils<DeformationFieldType>::readImage(deformationFilenames[intermediateID][targetID]);
                                    }else{
                                        deformation = deformationCache[intermediateID][targetID];
                                    }
                                    deformation=TransfUtils<ImageType,double>::linearInterpolateDeformationField(deformation, targetImage);
                                    ...
                                    firstDeformation=TransfUtils<ImageType,double>::linearInterpolateDeformationField(firstDeformation, intermediateImageIterator->second);
                                    ...
                                    if (weighting==UNIFORM || metric == NONE ){
                                        ...
                                        ImagePointerType atlasImage=(*atlasImages)[(*atlasIDMap)[atlasID]].second;
                                        if (weighting==GLOBAL){
                                            updateProbabilisticSegmentationGlobalMetric(probabilisticTargetSegmentation,probAtlasSegmentation,weight,targetImage,atlasImage,fullDeformation,metric);
                                        }else if (weighting==LOCAL){
                                            updateProbabilisticSegmentationLocalMetricNew(probabilisticTargetSegmentation,probAtlasSegmentation,weight,targetImage,atlasImage,fullDeformation,metric);
                                        }
                ...
                if (graphCut)
                    outputImage=probSegmentationToSegmentationGraphcutMultiLabel(probabilisticTargetSegmentation,targetImageIterator->second,smoothness*inputAtlasSegmentations->size(),m_graphCutSigma);
                else
                    outputImage=probSegmentationToSegmentationLocal(probabilisticTargetSegmentation);
                ostringstream tmpSegmentationFilename;
                tmpSegmentationFilename<<outputDir<<"/segmentation-weighting"<<weightingName<<"-metric"<<metricName<<"-target"<<targetID<<"-hop1"<<suffix;
...
#else
	LOG<<"GCO not available, result will be empty (and broken)."<<endl;
#endif
        return result;
    }
