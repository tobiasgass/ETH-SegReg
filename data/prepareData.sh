...
for n in `seq  2 $N`
do
    $binDir/DeformImage2D --moving Images/img-1.png --def DeformationFields/def-1-$n.mha --out Images/img-$n.png
    $binDir/DeformImage2D --moving Segmentations/seg-1.png --def DeformationFields/def-1-$n.mha --out Segmentations/seg-$n.png --NN
...
do
    echo $n >>List.IDs
    echo $n `pwd`/Images/img-$n.png >>List.Images
    echo $n `pwd`/Segmentations/seg-$n.png >>List.Segmentations
